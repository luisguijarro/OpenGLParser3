...
namespace OpenGLParser
{
    ...
    {
        private static void WriteDelegateInitializerEXT(string NameSpace, string outpath, bool verbose)
        {
            if (verbose) //Si Verbose mode mostramos inicio del proceso.
            {
                Console.WriteLine(); Console.WriteLine("Generating File: OpenGLInitDelsExts.cs");
            }
            if (!Directory.Exists(outpath)) // Si la ruta no existe la creamos
            {
                Directory.CreateDirectory(outpath);
            }
            if (File.Exists(outpath + "OpenGLInitDelsExts.cs")) //Si existe algun archivo previo lo eliminamos.
            {
                File.Delete(outpath + "OpenGLInitDelsExts.cs");
            }
            StreamWriter file = File.CreateText(outpath + "OpenGLInitDelsExts.cs"); //Generamos Contenido del archivo.
            file.WriteLine("// OpenGL Delegates Initializer for Extensions.");
            ...
            file.WriteLine();
            file.WriteLine(tab+tab+"internal static void InitDelegatesExts()"); //Declaramos Metodo Estatico Iniciador de Delegados..
            ...
            file.WriteLine(tab+tab+tab+"{");
            file.WriteLine(tab+tab+tab+tab+"glGetIntegerv = (dglGetIntegerv)"+NameSpace+".OpenGL.InternalTool.GetGLMethodAdress(\"glGetIntegerv\", typeof(dglGetIntegerv));");
            file.WriteLine(tab+tab+tab+tab+"glGetStringi = (dglGetStringi)"+NameSpace+".OpenGL.InternalTool.GetGLMethodAdress(\"glGetStringi\", typeof(dglGetStringi));");
            ...
            for (int key = 0;key<CommandsKeysList.Count;key++) //Recorremos la lista de Comandos
            {
                ...
                if (commandTemp.FromVersion.Length <= 0) //Solo escribimos los que NO estan en el Core de OpenGL.
                {
                    ...
                    string s_initDel = tab + tab + tab + tab + NameSpace + ".OpenGL.internalGL." + CommandsKeysList[key] + " = ";
            ...
            if (verbose) //Si Verbose mode mostramos la finalización del proceso.
            {
                ...
                Console.ResetColor(); 
                Console.WriteLine(": OpenGLInitDelsExts.cs");
            }
        }
    }
}
